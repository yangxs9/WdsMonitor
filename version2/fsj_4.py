...
def order(mainAmout, amount1, amount2, amount3):
    ...
    if mainAmout >= amount1:
        order -= 1
    if mainAmout >= amount2:
        order -= 1
    if mainAmout >= amount3:
        order -= 1
    return str(order)
...
while True:
    time.sleep(interval)
    mainWds.refreshInfo()
    if mainWds.isChanged == False or len(mainWds.addedUserMoney) == 0:
        continue
    ...
    for user in mainWds.addedUserMoney:
        msg = msg + user + ' 刚刚支持了小北' + str(mainWds.addedUserMoney[user]) + '，'
    ...
    msg = msg + '我们现在是第' + order(mainWds.amount, other1Wds.amount, other2Wds.amount, other3Wds.amount) + '名，'
    msg += '请大家为我们的黑夜女神小北奉上我们的力量吧，微打赏链接：' + main_url
